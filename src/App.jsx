...
export default function VinylCrate() {
  ...
  const openRecord = (id) => { window.location.hash = `/record/${encodeURIComponent(id)}`; };
  ...
  async function submitForm(e) {
    ...
    try {
      ...
      const extraDiscsNormalized = extraDiscsField.map((d) =>
        d.finish === "Picture disc" ? { ...d, colorLabel: "", colorHex: "" } : d
      );
      ...
      const rec = {
        ...
        colorLabel: form.finish === "Picture disc" ? "" : form.colorLabel,
        colorHex: form.finish === "Picture disc" ? "" : form.colorHex,
      ...
      <div className="vc-nav-glass">
        <header className="vc-header">
          ...
          <div className="vc-header-actions">
            {view === "grid" && activeTab !== "history" && activeTab !== "insights" && scopedRecords && scopedRecords.length > 0 && (
              <span className="vc-tally">{scopedRecords.length} {activeTab === "wishlist" ? "wanted" : "pressing"}{scopedRecords.length === 1 ? "" : "s"}</span>
            )}
            {view === "grid" && activeTab !== "history" && activeTab !== "insights" && (
              session ? (
                <button
              ...
              <span className="vc-sleeve-stage">
                <span className="vc-sleeve-cover">
                  ...
                  <span className="vc-sleeve-no">
                    ...
                    {isRecentlyAdded(r) && <em className="vc-new-badge">NEW</em>}
                  </span>
                  {(() => {
...
function DiscFace({ record }) {
  ...
  useEffect(() => {
    ...
    setDiscLoaded(false);
    const t = setTimeout(() => setDiscLoaded(true), 4000);
    return () => clearTimeout(t);
  }, [discImg]);
  if (discImg) {
...
function CoverArt({ coverUrl, artist, title, hex, alt }) {
  ...
  useEffect(() => {
    setFailed(false);
    // If the image is already in the browser cache, onLoad may never fire
    // (a known Safari behavior) — check completeness directly.
    ...
    setLoaded(false);
    // Failsafe: reveal the image after 4s even if load events never fire.
...
/* ---------- NEW badges ---------- */
.vc-new-badge {
  font-style: normal; margin-left: 6px; padding: 1px 6px; border-radius: 3px;
  background: var(--accent); color: #fbfaf7; font-size: 0.58rem; letter-spacing: 0.06em;
}
...
.vc-insight-total strong { display: block; font-family: 'Spectral', serif; font-size: 1.9rem; font-weight: 600; line-height: 1.1; }
...
@media (max-width: 760px) {
  ...
  .vc-stage { max-width: 320px; margin: 0 auto; }
  .vc-actions { gap: 8px; }

...
const isOwnedRecord = (record) => (record.status || "owned") === "owned";
...
export default function VinylCrate() {
  ...
  const wishlistRecords = useMemo(() => (records ? records.filter((r) => r.status === "wishlist") : []), [records]);
  const scopedRecords = records === null ? null : activeTab === "wishlist" ? wishlistRecords : ownedRecords;
  ...
  const distinctSizes = useMemo(() => {
    ...
    return [
      ...VINYL_SIZES,
      ...Array.from(set).filter((size) => !VINYL_SIZES.includes(size)),
    ];
  }, [scopedRecords]);
      ...
      <div className="vc-nav-glass">
        ...
        {view === "grid" && (
          <nav className="vc-tabs">
            ...
            >
              Collection <span className="vc-tab-count">{collectionCount}</span>
            </button>
            <button
...
function AboutView({ records, theme }) {
  ...
  const galleryItems = useMemo(() => {
    if (!records) return [];
    return [...records]
      .filter((r) => isOwnedRecord(r) && r.coverUrl)
      .sort(
        (a, b) =>
...
function HistoryView({ loading, records, onOpen }) {
  const ordered = useMemo(
    () => records.filter(isOwnedRecord).sort((a, b) => b.addedAt - a.addedAt),
    [records]
  );
  ...
  return (
    <main className="vc-main">
      <div className="vc-toolbar">
        ...
        <button
          type="button"
          className={`vc-icon-btn vc-filter-toggle ${filtersOpen || activeFilterCount > 0 ? "is-active" : ""}`}
        ...
        >
          ...
          Filters
          {activeFilterCount > 0 && <span className="vc-filter-count">{activeFilterCount}</span>}
          <ChevronDown className="vc-filter-chevron" size={14} strokeWidth={2} />
        </button>
      ...
      <AnimatePresence initial={false}>
        {filtersOpen && (
          ...
          >
            <div className="vc-filters-panel-inner">
              <div className="vc-filters-panel-head">
                <span>Filter your vault</span>
                {activeFilterCount > 0 && (
                  <button type="button" className="vc-filter-clear" onClick={clearAllFilters}>
                    Clear all
                  </button>
                )}
              </div>
              ...
              <span className="vc-sleeve-stage">
                <span className="vc-sleeve-cover">
                  ...
                  {(() => {
                    const badges = [
                      ...(r.specialTags || []),
                      ...(r.vinylSize && r.vinylSize !== "12\"" ? [r.vinylSize] : []),
                    ];
...
function SizeIcon({ vinylSize, size = 22 }) {
  const radiusBySize = { '7"': 8, '10"': 11, '12"': 13 };
  const r = radiusBySize[vinylSize] || 13;
  return (
...
function SpotifyIcon({ size = 18 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true" focusable="false">
      ...
      <path d="M6.8 9.6c3.6-1 7.9-.7 10.8.9" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
      <path d="M7.1 12.8c3-.8 6.5-.5 9 .9" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" />
      <path d="M7.5 15.8c2.4-.6 5.1-.4 7.1.7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
    </svg>
  );
}
...
function YouTubeMusicIcon({ size = 18 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true" focusable="false">
      <circle cx="12" cy="12" r="10.25" stroke="currentColor" strokeWidth="1.5" />
      <circle cx="12" cy="12" r="6.1" stroke="currentColor" strokeWidth="1.5" />
      <path d="M10.3 9.2l4.4 2.8-4.4 2.8z" fill="currentColor" />
    </svg>
  );
}
        ...
        <div className={`vc-stage ${revealed ? "is-revealed" : ""}`}>
          <div className="vc-stage-sleeve">
            ...
            {(() => {
              const badges = [
                ...(record.specialTags || []),
                ...(record.vinylSize && record.vinylSize !== "12\"" ? [record.vinylSize] : []),
              ];
...
.vc-filter-count {
  ...
  background: var(--accent); color: var(--paper); font-size: 0.62rem;
}
.vc-filters-panel { overflow: hidden; margin: -2px 0 18px; }
.vc-filters-panel-inner {
  padding: 16px 16px 2px; background: var(--panel); border: 1px solid var(--line); border-radius: 10px;
}
.vc-filters-panel-head {
  display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 14px;
  color: var(--ink); font-family: 'IBM Plex Mono', monospace; font-size: 0.7rem;
  letter-spacing: 0.06em; text-transform: uppercase;
}
.vc-filter-clear {
  background: transparent; border: none; padding: 3px 0; color: var(--accent); cursor: pointer;
  font-family: 'IBM Plex Mono', monospace; font-size: 0.66rem; text-transform: uppercase; letter-spacing: 0.04em;
}
.vc-filter-clear:hover { text-decoration: underline; text-underline-offset: 3px; }
.vc-filter-clear:focus-visible { outline: 2px solid var(--accent); outline-offset: 3px; border-radius: 2px; }
.vc-filters-panel .vc-chip-group:last-child { margin-bottom: 12px; }
...
@media (max-width: 760px) {
  ...
  .vc-filter-toggle { flex: 0 0 auto; }
  .vc-filters-panel-inner { padding: 14px 12px 1px; }
...
@media (prefers-reduced-motion: reduce) {
  .vc-sleeve, .vc-sleeve-disc, .vc-stage-disc, .vc-disc-layer, .vc-disc-face, .vc-spin-slow, .vc-filter-chevron { animation: none !important; transition: none !important; }
}
`;
